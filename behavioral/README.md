...
*   [**비지터 (Visitor)**](./Visitor) - 객체 구조를 변경하지 않고 새로운 연산 추가
*   [**중재자 (Mediator)**](./Mediator) - 객체 간 복잡한 통신을 중재자로 캡슐화
...
| Visitor | 연산 추가 | 복잡한 객체 구조에 다양한 연산 추가 | ⭐⭐⭐⭐ |
| Mediator | 통신 중앙화 | 다수 객체 간 복잡한 상호작용 | ⭐⭐⭐ |
...
### 🎯 비지터 패턴
- **컴파일러**: AST 순회하며 타입 검사, 코드 생성
- **파일 시스템**: 디렉터리 구조 순회하며 크기 계산, 검색
- **문서 처리**: XML/JSON 파싱 후 다양한 형식으로 변환

### 🎯 중재자 패턴
- **채팅 시스템**: 채팅방이 사용자 간 메시지 중재
- **UI 폼**: 폼 컴포넌트 간 상호작용 조율
- **항공 관제**: 관제탑이 비행기 간 통신 중재
...
### 객체 구조를 변경하지 않고 새 연산을 추가해야 할 때
→ **Visitor 패턴** 사용

### 다수 객체 간 복잡한 통신을 단순화해야 할 때
→ **Mediator 패턴** 사용

## ❌ 언제 사용하지 말 것

### Observer를 피해야 하는 경우
- 구독자가 하나뿐인 경우 (직접 호출이 더 간단)
- 이벤트 순서가 중요한 경우 (순서 보장 어려움)
- 동기적 즉시 응답이 필요한 경우 (비동기 특성)

### Strategy를 피해야 하는 경우
- 알고리즘이 하나뿐이고 변경 가능성이 없는 경우
- 전략 간 차이가 매우 미미한 경우 (if문이 더 간단)
- 전략 객체 생성 비용이 높고 자주 변경되는 경우

### Template Method를 피해야 하는 경우
- 알고리즘 변형이 거의 없는 경우 (단순 구현으로 충분)
- 상속 계층이 이미 복잡한 경우 (더 복잡해짐)
- 런타임에 알고리즘을 변경해야 하는 경우 (Strategy 사용)

### Command를 피해야 하는 경우
- 단순한 요청-응답만 필요한 경우 (직접 호출이 간단)
- 실행 취소나 로깅이 필요 없는 경우
- 커맨드 객체 생성 오버헤드가 문제되는 경우

### State를 피해야 하는 경우
- 상태가 2-3개 이하이고 전이가 단순한 경우 (if-else로 충분)
- 상태 간 공유 로직이 많은 경우 (중복 코드 발생)
- 상태 변경이 거의 없는 경우 (과도한 설계)

### Chain of Responsibility를 피해야 하는 경우
- 핸들러가 하나뿐인 경우 (체인 불필요)
- 모든 요청이 반드시 처리되어야 하는 경우 (처리 안 될 수 있음)
- 체인 순서가 자주 변경되는 경우 (관리 복잡)

### Visitor를 피해야 하는 경우
- Element 클래스가 자주 추가되는 경우 (모든 Visitor 수정 필요)
- Element 구조가 단순한 경우 (오버엔지니어링)
- 캡슐화가 중요한 경우 (내부 상태 노출)

### Mediator를 피해야 하는 경우
- 객체 간 관계가 단순한 경우 (직접 참조가 더 명확)
- 중재자 로직이 너무 복잡해지는 경우 (분리 필요)
- 실시간 성능이 중요한 경우 (간접 호출 오버헤드)
...
- **Visitor**: 새 Element 추가 시 모든 Visitor 수정 필요
- **Mediator**: 중재자가 God Object가 되지 않도록 주의
