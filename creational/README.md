...
### 추가 생성 패턴
*   [**추상 팩토리 (Abstract Factory)**](./AbstractFactory) - 관련된 객체들의 패밀리를 생성
*   [**프로토타입 (Prototype)**](./Prototype) - 기존 객체를 복사하여 새 객체 생성
...
| Abstract Factory | 관련 객체 패밀리 생성 | 플랫폼별 UI, 테마 시스템 | ⭐⭐⭐⭐ | ⭐⭐⭐ |
| Prototype | 객체 복제 | 복잡한 객체의 빠른 생성 | ⭐⭐ | ⭐⭐⭐ |
...
### 언제 Abstract Factory를 사용할까?
✅ **사용하는 경우**
- 관련된 객체들을 함께 생성해야 할 때
- 플랫폼/테마별로 다른 객체 패밀리가 필요할 때
- 제품 간 일관성을 보장해야 할 때

❌ **피해야 하는 경우**
- 제품 종류가 자주 추가되는 경우 (모든 팩토리 수정 필요)
- 제품 패밀리가 하나뿐인 경우 (Factory Method로 충분)
- 객체 간 관계가 느슨한 경우 (개별 팩토리가 더 유연)

### 언제 Prototype을 사용할까?
✅ **사용하는 경우**
- 객체 생성 비용이 복제보다 높을 때
- 런타임에 객체 타입을 결정해야 할 때
- 복잡한 초기화 과정을 피하고 싶을 때

❌ **피해야 하는 경우**
- 객체가 순환 참조를 가지는 경우 (깊은 복사 어려움)
- 복제할 필드가 final인 경우 (불변 객체)
- 객체 생성이 단순한 경우 (new가 더 명확)
